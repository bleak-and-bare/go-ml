...
func R2Score[T Number](trg, pred iter.Seq[T]) float64 {
	trg_var, _, n := stat.WelfordVar(trg)
...
func RMSE[T Number](trg, pred iter.Seq[T]) float64 {
	return math.Sqrt(float64(stat.Mean(adapter.Squared(vector.Diff(trg, pred)))))
}

// Mean absolute error
func MAE[T Number](trg, pred iter.Seq[T]) float64 {
	return float64(stat.Mean(iterable.Map(vector.Diff(trg, pred), func(v T) T {
